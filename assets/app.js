(() => {
  ...
  function narrativeHtml(value) {
    return escapeHtml(value)
      .replaceAll("[[PUBLIC_ADDRESS]]", '<mark class="public-address" title="Public project address retained after activity-aware screening">')
      .replaceAll("[[/PUBLIC_ADDRESS]]", "</mark>")
      .replaceAll("[REDACTED — POTENTIAL SINGLE-FAMILY ADDRESS]", '<span class="pii-redaction">[REDACTED — POTENTIAL SINGLE-FAMILY ADDRESS]</span>');
  }

  function truncateText(value, maxLength = 220) {
    const text = narrativePlainText(value);
  ...
  async function ensureNarrativeYears(years) {
    ...
    for (const year of needed) {
      ...
      for (const entry of entries) {
        ...
        if (DD.narrativeChunks) delete DD.narrativeChunks[entry.key];
      }
      narrativeYearsLoaded.add(year);
    }
  }

  async function collectNarratives(indices, limit = 40) {
    ...
    const selected = [...latestById.values()].sort((a, b) => b.quarter - a.quarter || b.id - a.id).slice(0, limit);
  ...
  function bindQuickElements() {
    ...
    quick.elements = {
      ...
      narrativeOnly: byId("qr-narrative-only"),
      ...
      narrativeSection: byId("qr-narrative-section"),
      narrativeColumns: byId("qr-narrative-columns"),
  ...
  function updateQuickScenarioChoices(key) {
    ...
    for (const row of ROWS) {
      ...
      if (quick.state.narrativeOnly && row[C.hasNarrative] !== 1) continue;
  ...
  function syncQuickSharedState() {
    ...
    quick.state.narrativeOnly = quick.elements.narrativeOnly.checked;
  ...
  function bindQuickEvents() {
    ...
    quick.elements.narrativeOnly.addEventListener("change", () => {
      quick.state.narrativeOnly = quick.elements.narrativeOnly.checked;
      for (const key of ["a", "b"]) {
        quick.state.scenarios[key].geographyValue = -1;
  ...
  async function resetQuickReport() {
    ...
    quick.state.narrativeOnly = false;
    ...
    quick.elements.metric.value = String(quick.state.metric);
    quick.elements.plotBasis.value = quick.state.plotBasis;
    quick.elements.narrativeOnly.checked = false;
    ...
    quick.elements.narrativeSection.hidden = true;
    quick.elements.narrativeColumns.replaceChildren();
  ...
  function summarizeQuickScenario(result, metricIndex) {
    ...
    for (const rowIndex of result.indices) {
      ...
      narrativeRecords += row[C.hasNarrative] === 1 ? 1 : 0;
    ...
    return {
      ...
      narrativeRecords,
      narrativeShare: result.indices.length ? narrativeRecords / result.indices.length : 0,
  ...
  function buildQuickTakeaways(report) {
    ...
    if (report.reportType === "single") {
      ...
      takeaways.push(`${formatInteger.format(report.summaryA.narrativeRecords)} displayed records (${formatPercent.format(report.summaryA.narrativeShare)}) have linked privacy-screened narratives.`);
      ...
      takeaways.push(`Linked privacy-screened narratives cover ${formatPercent.format(report.summaryA.narrativeShare)} of Scenario A records and ${formatPercent.format(report.summaryB.narrativeShare)} of Scenario B records.`);
  ...
  function quickKpis(report) {
    if (report.reportType === "single") {
      return [
        ...
        ["Narrative-linked", formatPercent.format(report.summaryA.narrativeShare), `${formatInteger.format(report.summaryA.narrativeRecords)} records`],
  ...
  function narrativeCardMarkup(title, rows, className = "") {
    const content = rows.length
      ? `<div class="quick-narrative-list">${rows.map((row) => `<article class="quick-narrative-item"><span class="quick-narrative-meta">${escapeHtml(row.quarter)} · ${escapeHtml(row.grantee)} · ${escapeHtml(row.title || row.type)}</span>${narrativeHtml(row.text)}</article>`).join("")}</div>`
      : '<div class="quick-narrative-empty">No linked sanitized narrative excerpts are available for this scenario.</div>';
    return `<div class="quick-narrative-card ${className}"><h5>${escapeHtml(title)}</h5>${content}</div>`;
  }

  function renderQuickNarrativeSection(report) {
    const cards = [narrativeCardMarkup(report.reportType === "comparison" ? "Scenario A" : report.labelA, report.narrativesA.rows, "scenario-a")];
    if (report.reportType === "comparison") cards.push(narrativeCardMarkup("Scenario B", report.narrativesB.rows, "scenario-b"));
    quick.elements.narrativeColumns.classList.toggle("single", report.reportType !== "comparison");
    quick.elements.narrativeColumns.innerHTML = cards.join("");
    quick.elements.narrativeSection.hidden = false;
  }

  function quickNarrativeBriefLines(report) {
    const lines = [];
    const add = (label, collection) => {
      for (const row of collection.rows.slice(0, report.reportType === "comparison" ? 1 : 2)) {
        const text = truncateText(row.text, report.reportType === "comparison" ? 145 : 175);
        lines.push(`${label} · ${row.quarter} · ${row.title || row.type}: ${text}`);
      }
    };
    add(report.reportType === "comparison" ? "A" : "Narrative", report.narrativesA);
    if (report.reportType === "comparison") add("B", report.narrativesB);
    return lines;
  }
  ...
  async function renderQuickReport(report) {
    ...
    annotations.push(
      ...
      { x: 0.525, y: 0.568, xref: "paper", yref: "paper", text: "<b>Sanitized Narrative Highlights</b>", showarrow: false, xanchor: "left", yanchor: "bottom", font: { size: 11.5, color: "#263449" } },
    ...
    shapes.push(
      { type: "rect", xref: "paper", yref: "paper", x0: 0.515, x1: 0.98, y0: 0.585, y1: 0.765, line: { color: "#d7e0ea", width: 1 }, fillcolor: "#f8fafc", layer: "below" },
      { type: "rect", xref: "paper", yref: "paper", x0: 0.515, x1: 0.98, y0: 0.40, y1: 0.555, line: { color: "#b9d9d5", width: 1 }, fillcolor: "#f0fdfa", layer: "below" },
    );
    const takeawayText = report.takeaways.slice(0, 3).map((item) => `• ${wrapAnnotationText(item, 70)}`).join("<br><br>");
    annotations.push({
      x: 0.532,
      y: 0.745,
      xref: "paper",
      ...
      yanchor: "top",
      align: "left",
      font: { size: 9.4, color: "#334155" },
    });
    const narrativeLines = quickNarrativeBriefLines(report);
    const narrativeText = narrativeLines.length
      ? narrativeLines.map((item) => `• ${wrapAnnotationText(item, 72)}`).join("<br><br>")
      : "No linked sanitized narrative excerpt is available for the selected records.";
    annotations.push({
      x: 0.532,
      y: 0.535,
      xref: "paper",
    ...
    const noteParts = [
      ...
      "Narrative excerpts are privacy-screened: public infrastructure/multifamily addresses may be highlighted, while potential single-family or ambiguous addresses are redacted.",
    ];
    annotations.push({
      x: 0.02,
      y: 0.015,
  ...
  async function generateQuickReport() {
    ...
    try {
      ...
      const narrativesA = await collectNarratives(resultA.indices, 3);
      const narrativesB = resultB ? await collectNarratives(resultB.indices, 3) : { total: 0, rows: [] };
      const report = {
        ...
        narrativesA,
        narrativesB,
        narrativeOnly: quick.state.narrativeOnly,
      ...
      renderQuickNarrativeSection(report);
  ...
  function downloadQuickReportCsv() {
    ...
    add("Only Records with Sanitized Narratives", report.narrativeOnly ? "Yes" : "No");
    add("");
    add("Scenario A");
    ...
    add("Narrative-linked Records", report.summaryA.narrativeRecords);
    add("Narrative-linked Share", report.summaryA.narrativeShare);
    if (report.reportType === "comparison") {
      add("");
      add("Scenario B");
      ...
      add("Narrative-linked Records", report.summaryB.narrativeRecords);
      add("Narrative-linked Share", report.summaryB.narrativeShare);
    ...
    add("");
    add("Privacy-screened Narrative Highlights");
    add("Scenario", "QPR Quarter", "Grantee", "Activity Title", "Activity Type", "Sanitized Narrative Excerpt");
    for (const row of report.narrativesA.rows) add("Scenario A", row.quarter, row.grantee, row.title, row.type, narrativePlainText(row.text));
    if (report.reportType === "comparison") for (const row of report.narrativesB.rows) add("Scenario B", row.quarter, row.grantee, row.title, row.type, narrativePlainText(row.text));
    ...
    add("This report-data export is aggregate only and contains no raw financial rows. Included narrative text is a privacy-screened excerpt.");
    add("Potential addresses in single-family, homeowner, buyout, rehabilitation/reconstruction, relocation, and ambiguous contexts are redacted before publication.");
      ...
      <article class="panel-shell ${className}" id="panel-${number}">
        ...
        <details class="filter-details" open>
          ...
          <div class="filter-body">
            <label class="checkbox-line"><input id="p${number}-narrative-only" type="checkbox"><span><strong>Only show records with sanitized narratives</strong><small>Uses the exact Grant + Activity Number + QPR quarter link. Potential single-family or ambiguous address text is redacted before publication.</small></span></label>
        ...
        <details id="p${number}-narrative-details" class="narrative-details">
          <summary>Linked privacy-screened narrative records</summary>
          <div class="narrative-body">
            <p class="narrative-privacy-legend"><span><mark class="public-address">Highlighted address</mark> retained for a public infrastructure/multifamily context</span><span><span class="pii-redaction">Redacted address</span> potential single-family or ambiguous PII</span></p>
            <p id="p${number}-narrative-status" class="narrative-status">Open this section to load the most recent linked sanitized narrative excerpts.</p>
            <div id="p${number}-narrative-table" class="table-scroll"></div>
          </div>
        </details>
      </article>`;
  }
  ...
  async function updateHierarchy(panel) {
    ...
    let running = state.narrativeOnly ? ROWS.filter((row) => row[C.hasNarrative] === 1) : [...ROWS];
  ...
  async function renderPanel(panel) {
    ...
    if (token === panel.renderToken && panel.elements.narrativeDetails.open) await renderNarratives(panel);
  }
  ...
  async function renderNarratives(panel) {
    ...
    panel.elements.narrativeStatus.textContent = "Loading recent privacy-screened narrative excerpts…";
    const collection = await collectNarratives(result.indices, 40);
    if (token !== panel.narrativeToken) return;
    if (!collection.total) {
      panel.elements.narrativeStatus.textContent = "No exact-key, nonempty sanitized narratives are linked to these records.";
      panel.elements.narrativeTable.replaceChildren();
      return;
    }
    panel.elements.narrativeStatus.textContent = `Showing the ${formatInteger.format(collection.rows.length)} most recent narrative records out of ${formatInteger.format(collection.total)} linked records. Address text has completed the public privacy-screening workflow.`;
    panel.elements.narrativeTable.innerHTML = `<table class="narrative-table"><thead><tr><th>QPR quarter</th><th>Grantee</th><th>Activity title</th><th>Activity type</th><th>Privacy-screened narrative excerpt</th></tr></thead><tbody>${collection.rows.map((row) => `<tr><td>${escapeHtml(row.quarter)}</td><td>${escapeHtml(row.grantee)}</td><td>${escapeHtml(row.title)}</td><td>${escapeHtml(row.type)}</td><td>${narrativeHtml(row.text)}</td></tr>`).join("")}</tbody></table>`;
  }
  ...
  function renderKpis(panel, result) {
    ...
    const cards = [
      ...
      ["Narrative-linked", formatPercent.format(narrativeShare), `${formatInteger.format(narrativeCount)} sanitized records`],
  ...
  function renderStatus(panel, result) {
    ...
    const narrativeText = panel.state.narrativeOnly ? " Only exact-key records with sanitized narratives are included." : "";
    panel.elements.status.innerHTML = `<strong>Analysis scope:</strong> ${escapeHtml(level.label)}; ${escapeHtml(scope)}. ${escapeHtml(geographyInterpretation(level) + methodText + narrativeText)}`;
  }
  ...
  function selectionContext(panel) {
    ...
    context["Only records with sanitized narratives"] = panel.state.narrativeOnly ? "Yes" : "No";
  ...
  function downloadAggregateCsv(panel) {
    ...
    try {
      ...
      const headers = [
        ...
        "Record Count", "Narrative-linked Records", "Unique Grants", "Unique Projects", "Unique Activities",
      ...
      const notes = [
        ...
        "This export is aggregate only and contains no raw financial rows or original address strings.",
      ];
  ...
  function renderHeader() {
    ...
    const badges = [
      ...
      `${formatInteger.format(metadata.redacted_potential_single_family_address_mentions || 0)} potential address mentions redacted`,
